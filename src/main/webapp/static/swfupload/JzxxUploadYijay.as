package {
	 ...
	 */
	public class JzxxUploadYijay extends Sprite {
		private var sizeLimit=5*1024*1024; // 2M上传限制（此参数可更改设置）
		private var _upURL="http://192.168.7.120:8080";  
		// 上传地址请设置成你的。如果swf放在和上传处理页面同一网站文件夹，请用相对地址。（此参数可更改设置）
		private var _numLimit=20;               // 最大允许文件数（此参数可更改设置）
		
		var param:Object = root.loaderInfo.parameters; // 取得FlashVars后面的所有参数
		...
		public function JzxxUploadYijay() {
			Security.allowDomain("*");
			Security.allowInsecureDomain("*");
			stage.scaleMode=StageScaleMode.NO_SCALE;   // 不拉伸
			_urlRequest=new URLRequest();
			_upURL +=param["urls"]==undefined?"":param["urls"];
			...
			ExternalInterface.addCallback("changeUrl",changeUrl);
			ExternalInterface.call("loadurl") ;
										  
		}
		
		private function changeUrl(urls:String){
			_urlRequest.url=urls;
			_upURL = urls;
			var _tmp_i=0;
		    while(_tmp_i<_imgShower.numChildren){
				_uploadingItem=YItem(_imgShower.getChildAt(_tmp_i));
				_uploadingItem.setRequestUrl(_upURL);
				_tmp_i++;
			}
		}
	...
	private function delImg(_tmp_index){// 按索引删除图片
			...
			while(_tmp_i<_tmp_num){// 重新排列剩下的图片,item索引index也要记得改变
				...
				_imgShower.getChildAt(_tmp_i).x=5+(_tmp_i%7)*(_UIWidth/7);
				_imgShower.getChildAt(_tmp_i).y=30+Math.floor(_tmp_i/7)*128;
				_tmp_i++;
				
				}
				
				if(_tmp_num==0){setEmptyState();}
				_statusTex.text="已经添加了 "+_fileQueue.length+" 张照片，还可以再添加 "+(_numLimit - _fileQueue.length)+" 张照片";
		...
		private function makeFoot(){
			...
			_pauseBu.x=_uploadBu.x+_uploadBu.width+10;
			_pauseBu.y=_uploadBu.y;
		...
		private function onFRL(e:Event){// 当文件浏览框的按钮
			...
			{   
				...
				if(!_fileQueue)// 文件待上传队列为空
				{
					...
					{
					notice("图片数量超过"+_numLimit+"张，本次操作保存前"+_numLimit+"张");
					_fileQueue.splice(_numLimit);     // 截断数组只保留限制长度
					}
					_tmp_num=_fileQueue.length; // 处理后的长度
					
					if(Math.ceil(_tmp_num/7)>2){// 超过2列
					...
					while(_tmp_i<_tmp_num){  // 遍历可添加的文件引用，放入_imgShower图片组显示容器
						...
						_tmp_file.x=5+(_tmp_i%7)*(_UIWidth/7); // 坐标计算
						_tmp_file.y=30+Math.floor(_tmp_i/7)*128 ;
					...
					_statusTex.text="已经添加了 "+_tmp_num+" 张照片，还可以再添加 "+(_numLimit - _tmp_num)+" 张照片";
						setUseState();
					 ...
					 if(_fileQueue.length==_numLimit)// 如果原本就满了
					 {
						 ...
						 if(_fileQueue.length+_tmp_list.length>_numLimit)// 旧有+新的总长度大于64
								...
								while(_tmp_i<_tmp_num){// 加到原有的待传队列之后
									...
									_tmp_file.x=5+((_tmp_i+_tmp_hasNum)%7)*(_UIWidth/7) ;
									_tmp_file.y=30+Math.floor((_tmp_i+_tmp_hasNum)/7)*128;
		...
		private function onUploadBu(e:Event){// 点击上传
			...
			if(_uploadingIndex<_imgShower.numChildren){
				...
				//_statusTex.text="正在上传第"+(_uploadingIndex+1)+"张图片(共"+_fileQueue.length+"张)";
				setUpState();
			}
			else{
				notice("没有可上传的图片");
			  ...
			  if(_tmp_index<_fileQueue.length){
				...
				_uploadingItem.upload();
				 ...
				 _statusTex.text="共"+_fileQueue.length+"张图片上传完毕";
					...
					if(!_foot.contains(_uploadBu))
						_foot.addChild(_uploadBu);
				ExternalInterface.call("setDesc",_fileQueue.length) ;
		...
		private function drawImgShower(_tmp_num) {// 根据缩略图张数重绘上传显示框大小
			var _tmp_height;
			//这里暂时修改成固定宽高：
			_UIHeight=518;//390+128
			
					
			if(Math.ceil(_tmp_num/7)<3){// 小于2行时 整个UI界面高390，上传显示框高为256
				_tmp_height=_imgShowerH;
				//_UIHeight=390;
				//_foot.y=_UIHeight-_footH;
			}else if(Math.ceil(_tmp_num/7)>2){// 大于2行时，每多一行上传框高+128，同时下部操作界面:_foot移动
				_tmp_height=Math.ceil(_tmp_num/7)*128  ;
				//_UIHeight=390+(Math.ceil(_tmp_num/7)-2)*128 ;
				//_foot.y=_UIHeight-_footH;
			}
