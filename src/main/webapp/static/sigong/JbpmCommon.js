...
var Process = function(){
	...
	var renderCandidate=function(msg){
		...
		sts.push('<div class="userinfo"><input id="processsubit" type="button" value="提交" style="width:80px;height:40px;"></div>');
		sts.push('  </div>');
		sts.push('</div>');
		
		if($('#candidate').size()==0){
			$('body').append('<div id="candidate" style="text-align:left;" class="easyui-window" title="请选择下一环节办理人" ></div>');
...
$(function(){
//	$(document).delegate('.processsubmit','click',Process.init);
	var p = new Process();
