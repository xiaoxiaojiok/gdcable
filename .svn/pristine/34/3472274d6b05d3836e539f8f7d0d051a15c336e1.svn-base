...
function uploadComplete(file) {
	try {
		...
		if (this.getStats().files_queued > 0) {
			...
			if(uploadType == "album"){
				var html = "<font color='red'><b>所有文件上传完毕!</b></font><br/>"
					html += "您可以点击选择图片继续上传&nbsp;&nbsp;或&nbsp;&nbsp;<a href='/album/groupAlbum/enterAlbum?photoId="+photoId+"'>进入相册</a>"
				progress.setStatus(html);
			}else{
				progress.setStatus("<font color='red'><b>所有文件上传完毕!</b></font>");
			}
