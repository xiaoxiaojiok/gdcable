package {
	...
	*/
	public class YItem extends Sprite {
		...
		private var sizeLimit=5*1024*1024 //2M上传限制
		...
		public  var filterArray =new Array(".gif",".jpg",".jpeg",".png",".GIF",".JPG",".JPEG",".PNG") //默认允许的后缀
		public  var uploadOver=false  //上传发送完毕
		public  var fileR             //缩略图的文件引用
		private var  _border:Sprite;  //被选中框         
		public var isSelected         //被选中标量

		public function YItem(_fr,_upURL="http://localhost:8080",_sizeLimit=5*1024*1024) {
		...
		public function setRequestUrl(urls:String){
			_urlRequest.url=urls;//文件上传路径
		}
    ...
    private  function onOutCon(e:Event){//鼠标移到缩略图上将显示操作菜单
		...
		private function handleFileR() {//FileReference为单个文件引用类。用来处理单个文件这里为了简单而使用。
		    ...
		    if(filterArray.indexOf(fileR.type)==-1)//检查文件格式后缀名
			{    
			    loadOver=true
				typeError=true
